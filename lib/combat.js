...
export function combat() {
  ...
  if (!enemy.action) {
    console.log(`${player.name} has ${player.effectiveness} effectiveness.`)
    console.log(`${enemy.name} has ${enemy.effectiveness} effectiveness.`)
...
function determineAttackType(attacker) {
  const numAttacks = Object.keys(attacker.weapon.attacks)
  if (numAttacks.length === 1) {
    return attacker.weapon.attacks[Object.keys[0]]
  }
  if (numAttacks.length > 1) {
    return attacker.weapon.attacks
  }
}
